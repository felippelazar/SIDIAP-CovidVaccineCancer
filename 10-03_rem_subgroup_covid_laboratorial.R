# ============================================================================ #
# 7. REM Analysis - COVID-19 Vaccine 3rd dose - Hosp Test until 3 day          #
...
ifelse(!dir.exists(here('Results', 'dose_3', 'sub_group_covid_lab')), dir.create(here('Results', 'dose_3', 'sub_group_covid_lab')), FALSE)

# Loading Auxiliary Objects for Analayis
source('aux_objects_rem_3.R')
current_analysis <- 'sub_group_covid_lab'
...
covid_date_vars <- colnames(covidCancerDiagnosisWide_subgroup)[grepl('covid_date_', colnames(covidCancerDiagnosisWide_subgroup))]

dfREMlong <- dfREMlong %>%
  select(-covid_date, -covid_death_date) %>%
  left_join(covidCancerDiagnosisWide_subgroup, by = c('subject_id')) %>%
  # NA occurrences for outcomes before minimum Date
  mutate(across(starts_with('covid_date'), ~ if_else(.x <= enrol_date, as.Date(NA), .x))) %>%
  ...
  select(-starts_with('covid_date_'))
