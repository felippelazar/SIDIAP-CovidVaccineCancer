# ============================================================================ #
# 9. REM Analysis - COVID-19 Vaccine 1st and 2nd - Subgroup Without Jansen     #
...
ifelse(!dir.exists(here('Results', 'dose_12', 'sub_group_not_jansen')), dir.create(here('Results', 'dose_12', 'sub_group_not_jansen')), FALSE)

# Loading Auxiliary Objects for Analayis
source('aux_objects_rem_12.R')
current_analysis <- 'sub_group_not_jansen'
...
dfREMVac <- dfREM %>%
  filter(coalesce(gv_vac_concept_id_1 != 'Jansen-Ad26', T)) %>%
  filter(coalesce(gv_vac_concept_id_2 != 'Jansen-Ad26', T)) %>%
