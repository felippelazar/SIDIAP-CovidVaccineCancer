# ============================================================================ #
# 4. Rolling Entry Matching Cohort Analysis  COVID-19 Vaccine 3rd  (Booster)   #
...
# Minimum date will be when the patient or control would be elegible for third dose booster vaccine. 
...
cancerREM_dose3 <- cancerMerged %>%
  ...
  mutate(
    ...
    vac_scheme_min_lag_days_12 = case_when(
      vac_scheme == 'Pfizer-mRNA-BNT162b-Pfizer-mRNA-BNT162b' ~ 21 + 30,
      vac_scheme == 'Moderna-mRNA-1273-Moderna-mRNA-1273' ~ 28 + 30,
      vac_scheme == 'AZ-ChAdOx1-AZ-ChAdOx1' ~ 90 + 30),
    vac_scheme_min_lag_days_23 = case_when(
      vac_scheme == 'Pfizer-mRNA-BNT162b-Pfizer-mRNA-BNT162b' ~ 180,
      vac_scheme == 'Moderna-mRNA-1273-Moderna-mRNA-1273' ~ 180,
      vac_scheme == 'AZ-ChAdOx1-AZ-ChAdOx1' ~ 90)
  ) %>%
...
cancerREM_dose3 <- cancerREM_dose3 %>%
  # Cancer at least Greater than Previous Dose Vaccination - 5 years
  filter(cancer_diag_first_date >= as.Date.character('27/12/2020', format = '%d/%m/%Y')-5*365) %>%
  # Patients with two-doses vaccination scheme
  filter(!is.na(vac_exposure_date_2)) %>%
  # Patients that had homologous combination vaccination scheme on 1st and 2nd vaccine doses with standard timing between doses
  filter(vac_scheme %in% 
           c('Pfizer-mRNA-BNT162b-Pfizer-mRNA-BNT162b', 'Moderna-mRNA-1273-Moderna-mRNA-1273', 'AZ-ChAdOx1-AZ-ChAdOx1')) %>%
  filter(vac_lag_days_2 <= vac_scheme_min_lag_days_12) %>%
  # Patients that received the 3rd dose after the eligibility date
  filter(coalesce(vac_exposure_date_3 >= minDate, T)) %>%
  # Patient alive on third dose eligibility date
  filter(coalesce(death_date >= minDate, T)) %>%
  # Patient active on the DB on eligibility date
  filter(coalesce(end_db_followup >= minDate, T)) %>%
  # Excluding patients with previous infection to eligibility date
  filter(coalesce(covid_date_1 >= minDate, T)) %>%
  # Excluding patients with previous hospitalization to eligibility date
  filter(coalesce(hosp_admission_date_1 >= minDate, T)) %>%
  # Filtering by Care Home Living
  filter(is.na(living_care_home)) %>%
  select(-vac_scheme_min_lag_days_12, -vac_scheme_min_lag_days_23)

#-- Factoring Gender Variable
cancerREM_dose3 <- cancerREM_dose3 %>%
...
# Create loop for day from all dates of available 3rd dose vaccination (booster)
date_list <- cancerREM_dose3 %>% arrange(vac_exposure_date_3) %>%
  drop_na(vac_exposure_date_3) %>% pull(vac_exposure_date_3) %>% unique()

# Create list to assign daily datasets to posteriorly combine them
z_merge_3rd_sens <- list()
eligibles_3rd_sens <- list()
...
for(j in 1:(length(date_list))){
  ...
  allCohort <- dfz %>%
    ...
    # Checking previous COVID-19 occurrences (Hospitalizations or Positive Tests)
    mutate(previous_covid = as.numeric(coalesce(covid_date <= startDate, FALSE)),
           ...
           previous_vac_3 =  as.numeric(coalesce(vac_exposure_date_3 < startDate, FALSE)),
   ...
   left_join(dfz %>%
    ...
    filter(cancer_diag_first_date > vac_exposure_date_1 - 5*365) %>%
    filter(cancer_diag_first_date < vac_exposure_date_1) %>%
    ...
    mergedCohort <- mergedCohort %>%
      filter((gc_outcome_vac_date_3 > gv_vac_exposure_date_3 | is.na(gc_outcome_vac_date_3)))
    
    ## Event for control (COVID-19 infection) occurs before paired vaccination
    # Note: For daily matching, there should not be any exclusion at this point as data was already filtered in up-steps
    mergedCohort <- mergedCohort %>%
      filter((gc_covid_date > gv_vac_exposure_date_3 | is.na(gc_covid_date)))
    
    ## Event for control (hospitalization) occurs before paired vaccination
    # Note: For daily matching, there should not be any exclusion at this point as data was already filtered in up-steps
    mergedCohort <- mergedCohort %>%
      filter((gc_hosp_admission_date > gv_vac_exposure_date_3 | is.na(gc_hosp_admission_date)))
    
    ## Event for control (death) occurs before paired vaccination
    # Note: For daily matching, there should not be any exclusion at this point as data was already filtered in up-steps
    mergedCohort <- mergedCohort %>%
      filter((gc_death_date > gv_vac_exposure_date_3 | is.na(gc_death_date)))
    ...
    if(nrow(mergedCohort) >= 1){
      ...
      z_merge_3rd_sens[[j]] <- mergedCohort
      rm(mergedCohort)
