...
NCO <- NCO[c(1:43, 56:60), ]

# Adding Non COVID-19 Death
nonCovDeath12 <- read.table(glue('Results/', 'dose_12/rem_main_analysis/outcome_noncovid_death_period_three.csv'), sep = ';', header = T) %>%
      select(term, estimate, std.error, conf.low, conf.high, n_event) %>%
      mutate(est_hr.conf.interval = sprintf('%.2f (%.2f - %.2f)', estimate, conf.low, conf.high)) %>%
      mutate(OutcomeName = 'Non-COVID-19 Death')

nonCovDeath3 <- read.table(glue('Results/', 'dose_3/rem_main_analysis/outcome_noncovid_death_period_three.csv'), sep = ';', header = T) %>%
      select(term, estimate, std.error, conf.low, conf.high, n_event) %>%
      mutate(est_hr.conf.interval = sprintf('%.2f (%.2f - %.2f)', estimate, conf.low, conf.high)) %>%
      mutate(OutcomeName = 'Non-COVID-19 Death')

# Getting Estimates with non COVID-19 Death
...
negOutcomes12 <- negOutcomes12 %>% left_join(NCO) %>% bind_rows(nonCovDeath12)
...
negOutcomes3 <- negOutcomes3 %>% left_join(NCO) %>% bind_rows(nonCovDeath3)

# Plot of Negative Control Outcome Estimates
ggplot(bind_rows(negOutcomes12, negOutcomes3) %>% filter(!is.na(estimate)) %>% filter((term != 'periodV1 0-14D') & (term != 'periodV3 0-14D')), 
      ...
      facet_grid(.~term) + scale_x_log10() + coord_cartesian(xlim=c(0.1,5)) + 
      theme_minimal() +
...
ggsave('Figures/NegOutcomesPlot_Figure_R3_all_outcomes.png', height = 150, width = 250, units = "mm", dpi = "print")

# Plotting Calibration
negOutcomes12_toPlot <- negOutcomes12 %>% filter(term == 'periodV1 14D+') %>%
      select(estimate, std.error) %>% filter(!is.na(estimate))
...
ggsave("Figures/NegOutcomes_CalibrationPlot_R3_all_outcomes.png", plot = tt, height = 260, width = 260, units = "mm", dpi = "print")
...
saveWorkbook(wb, glue('Results Tidy/',  'neg_outcomes_tidy_R3_all_outcomes.xlsx'), overwrite = TRUE)
